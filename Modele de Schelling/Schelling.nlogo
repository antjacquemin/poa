globals [
  ;; nombre de cellules sur la grille
  ;; (max-pxcor + 1) * (max-pycor + 1)
  ;; density-rate (slider dans l'interface)
  ;; tolerance (slider dans l'interface)
  nb-agents
  satisfaction-ratio
  aggregate-index
]
...
turtles-own [
state ;; yellow (-1) or green (+1)
required-tolerance
satisfied?
;; nombre d'agents voisins du m^eme type
similar-nearby
...
to setup
  ...
  ask patches [
    if (n > 0) [
      sprout 1
      [
        ifelse (n mod 2 > 0) [ set state -1 ] [ set state 1 ]
      ]
    ]
    set n (n - 1)
  ]
...
to update-satisfaction
  ask turtles [
    ...
    ifelse (social-degree != 0)
      [ let my-state state
        set other-nearby
        count (turtles-on neighbors) with [state != my-state]
        set required-tolerance (other-nearby / social-degree)
        set satisfied? (required-tolerance <= tolerance)
      ]
      [ set satisfied? true ]
  ]
...
to update-aggregate-index
  ...
  set aggregate-index (sim - 0.5) / 0.5
end
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>satisfaction-ratio</metric>
    <metric>aggregate-index</metric>
    <steppedValueSet variable="density-rate" first="0.3" step="0.1" last="0.9"/>
